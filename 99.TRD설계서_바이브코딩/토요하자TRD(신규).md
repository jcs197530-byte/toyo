...
flowchart TB
    ...
    subgraph L3["로직 계층"]
        C1["crud/*.py (도메인별 7개)<br/>CRUD, ban 기반 격리, 소프트 삭제"]
...
| 로직 | CRUD, ban 격리, 소프트 삭제, 해싱, 암호화, 중복/채번 검증 | crud/*, security.py, encryption.py | `user_id` 격리가 아니라 **`ban`(담당 반) 격리**, 삭제는 `use_yn='N'` UPDATE(소프트 삭제)만 존재 — 물리 삭제·CASCADE 없음(§9) |
...
3. **화면 계층에 관리자 화면 분리 추가**: §13에서 결정한 대로, 일반 교사용 `index.html`과 관리자 전용 `admin.html`(SCR-901, 기준정보 관리)을 화면 계층에서부터 분리했다. 원본에는 화면이 하나였다.
...
> 첨부된 `3-2. DDL(기존).md`를 `toyo테이블스키마DB_덤프.sql`(원본 컬럼 정의) 및 `phpcssjs소스.zip`(실제 WHERE/UPDATE 쿼리)과 전수 대조했다. **테이블 34개 전체의 컬럼명·타입·길이·NOT NULL은 프로그램으로 diff한 결과 전부 정확했다.** 문제는 컬럼 단위가 아니라 ①일부 테이블의 **PK 구성**, ②전체 FK에 잘못 들어간 **`ON DELETE CASCADE`**(물리 삭제가 존재하지 않는 프로젝트에 CASCADE 자체가 불필요), ③업무 테이블이 아닌 **`md5enc` 테이블 포함** 3가지였다. 아래에 근거와 함께 정리하고, 하단에 수정본 전체를 첨부한다.
...
### 검증 결과 4 (가장 중요) — FK에 `ON DELETE CASCADE`가 있으면 안 된다

기존 문서는 36개 FK 전부를 예외 없이 `CASCADE`로 통일했다. **이건 이 프로젝트에 애초에 성립할 수 없는 정책이다.** `CASCADE`는 "부모 행을 물리적으로 `DELETE`했을 때 자식 행도 함께 지운다"는 동작인데, §8-4·§9에서 실코드로 확인한 대로 **toyo는 레거시부터 지금까지 `toyo_*` 테이블에 물리 `DELETE`가 단 한 번도 실행된 적이 없고, 신규 시스템에서도 물리 삭제 기능 자체를 만들지 않는다.** 삭제 요청은 예외 없이 `use_yn='N'` UPDATE로만 처리된다. 실행될 일이 없는 `DELETE`를 전제로 한 `CASCADE`는 죽은 설정일 뿐 아니라, 만에 하나 운영자가 실수로 콘솔에서 직접 `DELETE`를 실행했을 때 오히려 대량 연쇄 삭제를 허용해 버리는 위험한 설정이다.

**CASCADE가 들어가 있었다면 실제로 얼마나 위험했을지 연쇄를 따라가 보면:**

```mermaid
flowchart TD
    A["DELETE FROM toyo_ban_code_name<br/>WHERE ban='003' (반 코드 실수로 삭제)"] --> B["ON DELETE CASCADE라면<br/>toyo_stud_info의 ban='003' 학생 전원 연쇄 삭제"]
    B --> C["그 학생들의<br/>toyo_stud_score 전부 연쇄 삭제"]
    B --> D["그 학생들의<br/>toyo_stud_chul(출석) 전부 연쇄 삭제"]
    B --> E["그 학생들의<br/>toyo_stud_dream/memory/bible_memory<br/>전부 연쇄 삭제"]
    B --> F["그 학생들의<br/>toyo_point/point_payment 전부 연쇄 삭제"]
    A -.->|"실제 설계(RESTRICT)"| G["FK 위반으로 즉시 차단<br/>(학생 데이터는 그대로 보존)"]
...
실제로 기존 PHP 코드에서도 "코드성 마스터"는 하드 `DELETE`가 아니라 `USE_YN='N'`(사용안함) 소프트 삭제로만 운영되고 있었다(`banCodeNameModify.php`, `banBibleContestCodeModify.php` 등에서 확인). 학생 소유 데이터(`stud_no` 참조) 쪽도 마찬가지로 물리 삭제 자체가 없으므로, "학생 데이터라서 CASCADE가 정상"이라는 논리도 성립하지 않는다 — **애초에 지워지는 부모 행이 없으므로 CASCADE든 RESTRICT든 발동할 일이 없는 것이 원칙이나, 방어적으로는 전부 `RESTRICT`로 통일해 실수로라도 연쇄 삭제가 나지 않도록 막는다.**

**수정 원칙**:

| 참조 대상 | 정책 | 이유 |
|---|---|---|
| `toyo_stud_info.stud_no` (학생 소유 트랜잭션 데이터: 출석/성적/드림/암송/포인트 등) | **RESTRICT** | 물리 삭제 자체가 없으므로 CASCADE 불필요. 실수로 DELETE를 시도해도 연쇄 삭제 대신 즉시 차단되는 것이 안전 |
...
| `toyo_team_game_score` ← `toyo_team_game_score_rows` (헤더-상세) | **RESTRICT** | 상세 행도 물리 삭제 대상이 아니므로 동일하게 CASCADE 불필요 |
...
-- 정책: REG_DATE/MOD_DATE는 TIMESTAMP로 통일. 물리 DELETE가 존재하지 않는 프로젝트이므로
--       모든 FK는 예외 없이 ON DELETE RESTRICT (CASCADE 미사용, § 하단 표 참고)
-- ============================================
...
-- ============================================
-- Foreign Key 제약
-- 원칙: 물리 DELETE가 존재하지 않는 프로젝트이므로(삭제 요청은 항상 use_yn='N' UPDATE),
--       CASCADE는 어떤 참조에도 사용하지 않는다. 전체 FK는 예외 없이 ON DELETE RESTRICT.
-- ============================================

-- [RESTRICT] 학생(stud_no) 소유 트랜잭션 데이터 — 물리 삭제가 없으므로 CASCADE 불필요
ALTER TABLE toyo_stud_2hakgi_info ADD CONSTRAINT fk_toyo_stud_2hakgi_info_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_2hakgi_new_info ADD CONSTRAINT fk_toyo_stud_2hakgi_new_info_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_new_info ADD CONSTRAINT fk_toyo_stud_new_info_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_reg_fee ADD CONSTRAINT fk_toyo_stud_reg_fee_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_field_attend ADD CONSTRAINT fk_toyo_stud_field_attend_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_bible_memory ADD CONSTRAINT fk_toyo_stud_bible_memory_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_chul ADD CONSTRAINT fk_toyo_stud_chul_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_dream ADD CONSTRAINT fk_toyo_stud_dream_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_dream_course ADD CONSTRAINT fk_toyo_stud_dream_course_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_dream_year_course ADD CONSTRAINT fk_toyo_stud_dream_year_course_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_memory ADD CONSTRAINT fk_toyo_stud_memory_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_score ADD CONSTRAINT fk_toyo_stud_score_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_stud_score_hist ADD CONSTRAINT fk_toyo_stud_score_hist_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_point ADD CONSTRAINT fk_toyo_point_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;
ALTER TABLE toyo_point_payment ADD CONSTRAINT fk_toyo_point_payment_stud_no FOREIGN KEY (stud_no) REFERENCES toyo_stud_info (stud_no) ON DELETE RESTRICT;

-- [RESTRICT] 상세 행 — 헤더(팀게임 점수)도 물리 삭제 대상이 아니므로 동일하게 CASCADE 불필요
ALTER TABLE toyo_team_game_score_rows ADD CONSTRAINT fk_toyo_team_game_score_rows_header FOREIGN KEY (enroll_date, kind, team) REFERENCES toyo_team_game_score (enroll_date, kind, team) ON DELETE RESTRICT;
...
│   └── router.py                 # 기준정보 CRUD 진입점(반/희망분야/과목 등 코드 테이블 관리)
│
...
| **`admin/` 신규 추가** | 없음 | 횡단 관리자 라우터 추가 | 9·13번 문서에서 결정한 "기준정보 관리는 관리자 전용, 403 검증" 기능이 들어갈 위치가 원본에 없었다 |
...
> ⚠️ **레거시에는 대응할 함수가 없음 (중요한 전제 차이)**: 레거시 PHP 256개 파일을 전수 확인한 결과, 서버 측 재사용 함수가 하나도 없는 순수 procedural 구조였다(화면 1개 = SQL+로직+출력이 한 파일에 뒤섞인 구조). 따라서 아래 함수들은 레거시를 그대로 옮긴 것이 아니라, 8·9·11·12·13번 문서에서 확정한 업무 규칙(반 기반 접근 제어, 소프트 삭제(use_yn) 전용 — 물리 삭제 없음, 성적 upsert, 채번 동시성 등)을 근거로 **신규로 설계**한 것이다.
...
| soft_delete_student(db, stud_no) | crud/student.py | 대상 stud_no | `use_yn='N'` UPDATE (유일한 삭제 경로 — 물리 DELETE는 어떤 함수에도 존재하지 않음) |
...
| **레거시 대비 신규 원칙** | `stud_no` 등 참조 컬럼에 실제 FK 제약을 건다. 기존에는 FK가 없어 학생을 삭제해도 출석·성적·암송 기록이 고아 행으로 남을 수 있었다(제약이 없으니 오류조차 나지 않는다) — 신규 설계에서는 물리 DELETE 자체를 사용하지 않으므로(§8-4·§9), FK는 전부 `ON DELETE RESTRICT`로 통일하고 CASCADE는 어디에도 사용하지 않는다(§3-2 DDL 참고) |
...
신규 시스템도 이 원칙을 그대로 따른다. **삭제 요청은 예외 없이 `use_yn='N'` UPDATE 하나뿐이며, 물리 `DELETE`를 실행하는 기능·API는 어떤 화면에도 만들지 않는다.** 이에 따라 FK에도 `ON DELETE CASCADE`를 둘 이유가 없다 — 자식 레코드가 연쇄로 사라지는 상황 자체가 설계상 발생하지 않아야 하므로, 전체 FK는 `ON DELETE RESTRICT`로 통일한다(§3-2 DDL 참고).

| 삭제 유형 | 적용 대상 | 동작 |
|---|---|---|
| **소프트 삭제 (유일한 삭제 경로, 레거시와 동일)** | 학생/반/교사 등 모든 CRUD | `use_yn`을 `'N'`으로 UPDATE, 물리 레코드는 항상 그대로 유지 |

관리자 전용 화면(SCR-901)도 기준정보(코드 테이블) CRUD만 제공하며, 물리 삭제(하드 삭제) 기능은 포함하지 않는다(§13-4 참고).
...
> 8번 문서와 마찬가지로, 레거시 PHP의 `alert()` 기반 오류 처리(HTTP 상태 코드 없이 클라이언트 스크립트로만 안내)를 FastAPI의 표준 상태 코드 체계로 재설계한 것입니다. toyo 프로젝트는 contact-manager 예시와 달리 **① 전체 FK가 RESTRICT로 통일(CASCADE 미사용)**되어 있고 **② 레거시와 동일하게 삭제는 소프트 삭제(`use_yn`)뿐, 물리 삭제 기능 자체가 없다**는 두 가지 특징이 상태 코드 설계에 직접 반영됩니다.
...
| 403 | 권한 없음 | 일반 교사(`AUTH`)가 관리자 전용 기능(기준정보 관리, 반 배정 변경 등) 호출 |
...
- **삭제는 소프트 삭제 하나뿐**: 모든 CRUD의 "삭제"는 `use_yn='N'` UPDATE이며 404/409를 발생시키지 않는다(존재만 확인하면 됨). 물리 `DELETE`를 실행하는 API·기능은 어디에도 없으므로, CASCADE로 인한 연쇄 삭제나 그로 인한 409 방지 로직 자체가 애초에 필요 없다 — 이 점이 contact-manager(카테고리 RESTRICT + 하드 삭제) 예시와 가장 크게 다른 부분이다.
...
# 6) 삭제(일반 교사 포함 모든 사용자 공통) — 물리 DELETE는 사용하지 않고 use_yn UPDATE만 실행
student = crud.get_student_in_my_ban(db, teacher.ban, stud_no)
if student is None:
    raise HTTPException(status_code=404, detail="해당 학생 정보가 없습니다")
crud.soft_delete_student(db, stud_no)  # UPDATE toyo_stud_info SET use_yn='N' — 물리 레코드는 그대로 보존
```
...
│   ├── dreams.py                  # 희망분야 등록
│   └── admin.py                   # 관리자 전용 — 기준정보(code_tables) 관리
│
...
| 삭제 정책 | RESTRICT 예외 있음, 하드 삭제 존재 | 전체 RESTRICT(CASCADE 미사용), 소프트 삭제(`use_yn`) 전용 — 물리 삭제 기능 자체가 없음 |
...
- [ ] 전체 FK 36개가 `ON DELETE RESTRICT`로만 선언되어 있고 `CASCADE`가 하나도 없는지 확인(`information_schema.referential_constraints`로 일괄 조회)
...
- [ ] **AUTH(권한) 기반 403 확인** — 일반 교사 계정으로 관리자 전용 API(기준정보 관리) 호출 시 403
- [ ] 로그아웃 후 동일 쿠키 401 확인
...
- [ ] **물리 삭제 API가 존재하지 않는지 확인** — 관리자 화면(SCR-901) 포함 어떤 라우터에도 `DELETE FROM toyo_*`를 실행하는 코드가 없는지, 실수로 상위 테이블(예: `toyo_stud_info`)에 직접 DELETE를 시도하면 RESTRICT로 차단되고 연쇄 삭제가 발생하지 않는지
...
> ⚠️ **행 [삭제]는 소프트 삭제(`use_yn='N'`)만 존재**: 레거시가 물리 삭제 없이 소프트 삭제만 써왔던 관행을 그대로 유지한다. 물리 삭제(하드 삭제) 기능 자체를 신규 시스템에도 만들지 않으며, 이는 일반 교사 화면뿐 아니라 **관리자 전용 화면(SCR-901)에서도 마찬가지**다(§9 참고).
...
### 13-4. SCR-901 관리자 전용 — 기준정보 관리 화면

- 반/희망분야/과목 등 기준정보(코드 테이블) CRUD
- 학생·성적·출결 등 업무 데이터에 대한 물리 삭제(하드 삭제) 기능은 제공하지 않는다 — 삭제는 일반 교사 화면과 동일하게 소프트 삭제(`use_yn='N'`)만 사용
